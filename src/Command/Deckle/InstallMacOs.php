...
class InstallMacOs extends AbstractDeckleCommand implements ProjectIndependantCommandInterface
{
...
END;
    ...
    protected function installBrew()
    {
        if (!$this->fs()->isInPath('brew')) {
            ...
            if ($this->confirm('Would you like to install <info>Homebrew</info> now?')) {
                $this->sh()->exec('/usr/bin/ruby -e "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/master/install)"',
                    null, false);
            } else {
    ...
    protected function installDocker()
    {
        ...
        $this->sh()->exec('brew install docker');
    ...
    protected function installMutagen()
    {
        ...
        $this->sh()->exec('brew install mutagen-io/mutagen/mutagen');
    }
    ...
    protected function installDnsmasq()
    {
        ...
        if ($dnsmasqInstalled) {
            ...
            $this->sh()->exec('brew install dnsmasq');
        ...
        foreach ($hosts as &$line) {
            if (preg_match('/(\d+\.\d+\.\d+\.\d+)\s+(.*?deckle-vm.*)/', $line, $matches)) {
                if ($matches[1] != $this->vm()->ip()) {
                    Deckle::print('Updating <info>/etc/hosts</info>');
                    $newLline = $this->vm()->ip() . "\t" . $matches[2] . "\n";
                    $line = $newLline;
    ...
    protected function installVm()
    {
        ...
        if (isset($return) && !$return->isErrored()) {
            Deckle::success([
                'Deckle Vagrant VM is up and running.'
            ]);
        } else {
